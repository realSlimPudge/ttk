...
export function ArticleHistory({ history, onClose }: ArticleHistoryProps) {
            ...
            <div className="space-y-4">
              {history.map((entry) => (
                <div key={entry.id} className="flex items-start p-4 border rounded-lg">
                  <div className="mr-4 mt-1">{getEventIcon(entry.eventType)}</div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <h4 className="font-medium">{entry.articleTitle}</h4>
                      <span className="text-sm text-muted-foreground">{formatDate(entry.timestamp)}</span>
                    </div>
                    <p className="text-sm font-medium text-muted-foreground mt-1">{getEventText(entry.eventType)}</p>
